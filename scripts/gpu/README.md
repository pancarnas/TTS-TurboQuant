...
- **`torchcodec` errors from `torchaudio.load`** (ModuleNotFoundError or
  "TorchCodec is required") — your torchaudio is ≥ 2.8, which moved audio
  decoding into the separate `torchcodec` package (which in turn needs FFmpeg
  ...
  To keep a newer torch instead: `pip install torchcodec "torch==<your version>"`
  plus `conda install -c conda-forge 'ffmpeg<8'` (or apt ffmpeg). `02_smoke.sh`
  probes a real decode in its preflight and attempts the torchcodec route
  automatically before failing with these instructions.
