"""Score divergence-experiment wavs: Whisper CER + WavLM speaker sim vs fp16.

``kv_attn_divergence_experiment.py`` saves one wav per (sentence, config) but
computes no perceptual metrics (its CSV holds attention/KV divergence only).
This standalone pass scores those wavs offline:

  * CER: Whisper (default large-v3, resampled to 16 kHz — same corrections as
    tools/rescore_audio.py) against the ground-truth text, for EVERY config;
  * SpkSim: WavLM x-vector cosine of each compressed wav against the SAME
    ...
    the same seed/temperature/sampling path). Baseline rows get CER but a blank
...
def parse_wav_name(name: str) -> dict | None:
    """Fields of one divergence-experiment wav filename, or None if foreign.

    The greedy ``group`` pattern still splits correctly because ``idx`` is the
    LAST ``_<digits>_sampling`` run before the fixed suffix, and group names
...
def collect_entries(audio_dir: str) -> tuple[list[dict], int]:
    ...
    for name in sorted(os.listdir(audio_dir)):
        ...
        out.append(e)
    out.sort(
        key=lambda e: (e["group"], e["idx"], e["seed"], e["temperature"], e["config"])
    )
    return out, foreign
...
def attach_baseline(entries: list[dict]) -> int:
    ...
    base = {
        (e["group"], e["idx"], e["seed"], e["temperature"]): e["path"]
        for e in entries
    ...
    for e in entries:
        if e["config"] == BASELINE:
            ...
            continue
        e["baseline_path"] = base.get(
            (e["group"], e["idx"], e["seed"], e["temperature"])
        )
...
def entry_key(e: dict) -> tuple:
    return (e["group"], e["idx"], e["config"], e["pl"])


def done_keys(out_path: str) -> set:
    """(group, idx, config, pl) already scored — for --resume."""
    if not os.path.exists(out_path) or os.path.getsize(out_path) == 0:
        return set()
    done = set()
    with open(out_path, newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            done.add((row["group"], int(row["idx"]), row["config"], int(row["pl"])))
    return done
...
def score_entries(
    ...
    orchestration is testable without Whisper/WavLM/soundfile. Baseline
    embeddings are computed once per sentence (entries are sentence-sorted).
    """
...
def summarize(out_path: str) -> None:
    ...
    for cfg, g in df.groupby("config"):
        ...
        print(
            f"{cfg:12s} n={len(g):4d}  meanCER {g['cer'].mean():.3f}  "
...
def main() -> None:
    from tools.rescore_audio import CERScorer, WhisperTranscriber, build_text_lookup
    ...
    with open(args.out, "a" if append else "w", newline="", encoding="utf-8") as fh:
        ...
        score_entries(
            entries, text_lookup, transcribe, score, embed, _wav_duration, emit
        )
    if bar:
        bar.close()
    summarize(args.out)
