...
from .turboquant_kv_cache import TurboQuantConfig, TurboQuantKVCache
    ...
    def generate(
        ...
        **kwargs,
        ...
        if turboquant_config is not None and turboquant_config.enabled:
            talker_cache = TurboQuantKVCache(
                turboquant_config,
                n_layers=self.config.talker_config.num_hidden_layers,
            )
        else:
            talker_cache = DynamicCache()
        ...
        # Store last cache for post-generation inspection (e.g. memory_report())
        self.last_kv_cache = talker_cache
